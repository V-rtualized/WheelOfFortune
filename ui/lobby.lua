...
function MP.UI.lobby_status_display()
	...
	if not bothPlayersInLobby then
		return result
	end

	local hostVersion = MP.LOBBY.host and MP.LOBBY.host.config and MP.LOBBY.host.config.Mods["WheelOfFortune"]
	local guestVersion = MP.LOBBY.guest and MP.LOBBY.guest.config and MP.LOBBY.guest.config.Mods["WheelOfFortune"]
...
function WOF.is_active()
	if not MP or not MP.LOBBY or not MP.LOBBY.code then
		return false
	end
	local hostVersion = MP.LOBBY.host and MP.LOBBY.host.config and MP.LOBBY.host.config.Mods["WheelOfFortune"]
	local guestVersion = MP.LOBBY.guest and MP.LOBBY.guest.config and MP.LOBBY.guest.config.Mods["WheelOfFortune"]
